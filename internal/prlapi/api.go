...
// PublicParallaxAPI provides an API to access Parallax related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicParallaxAPI struct {
	b Backend
}

// NewPublicParallaxAPI creates a new Parallax protocol API.
func NewPublicParallaxAPI(b Backend) *PublicParallaxAPI {
	return &PublicParallaxAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicParallaxAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicParallaxAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicParallaxAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
// Sign calculates an Parallax ECDSA signature for:
...
//
// https://github.com/ParallaxProtocol/parallax/wiki/Management-APIs#personal_sign
...
//
// https://github.com/ParallaxProtocol/parallax/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Parallax signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Parallax blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Parallax blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Parallax chain config.
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	defer func(start time.Time) { log.Debug("Executing PVM call finished", "runtime", time.Since(start)) }(time.Now())
	...
	// Get a new instance of the PVM.
	msg, err := args.ToMessage(globalGasCap, header.BaseFee)
	if err != nil {
		return nil, err
	}
	pvm, vmError, err := b.GetPVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true})
	if err != nil {
		return nil, err
	}
	// Wait for the context to be done and cancel the pvm. Even if the
	// PVM has finished, cancelling may be done (repeatedly)
	go func() {
		<-ctx.Done()
		pvm.Cancel()
	}()
	...
	result, err := core.ApplyMessage(pvm, msg, gp)
...
// revertError is an API error that encompassas an PVM revertal with JSON error
...
func (api *PublicBlockChainAPI) Call(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Bytes, error) {
	result, err := DoCall(ctx, api.b, args, blockNrOrHash, overrides, api.b.RPCPVMTimeout(), api.b.RPCGasCap())
	if err != nil {
		return nil, err
	}
...
func RPCMarshalHeader(head *types.Header) map[string]any {
	result := map[string]any{
		...
		"epochStartTime":   hexutil.Uint64(head.EpochStartTime),
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		vmenv, _, err := b.GetPVM(ctx, msg, statedb, header, &config)
		if err != nil {
			return nil, 0, nil, err
		}
...
// PublicDebugAPI is the collection of Parallax APIs exposed over the public
// debugging endpoint.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeLax := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Ether)))
	feeFloat, _ := feeLax.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f laxes) exceeds the configured cap (%.2f laxes)", feeFloat, cap)
	}
	return nil
}
