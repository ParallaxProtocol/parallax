...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom xhash engine.
	xhash := NewTester([]string{server.URL}, false)
	defer xhash.Close()
	...
	xhash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := xhash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	xhash := New(config, []string{server.URL}, false)
	defer xhash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom xhash engine.
	xhash := NewTester([]string{server.URL}, false)
	xhash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer xhash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			xhash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, xhash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch xhash result timeout", id+1)
		}
	}
}
