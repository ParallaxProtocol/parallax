...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the Prlx command line interface")
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend prlapi.Backend, isConsole bool) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		parallaxBackend.TxPool().SetGasPrice(gasprice)
		// start mining
		threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		if err := parallaxBackend.StartMining(threads); err != nil {
