...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Prlx.LightScryptN, Prlx.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.parallax", new NodeConfig());
			node.start();
			...
			ParallaxClient ec = node.getParallaxClient();
...
func TestAndroid(t *testing.T) {
	...
	// Generate the mobile bindings for Prlx and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.parallax", "github.com/ParallaxProtocol/parallax/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "prlx.aar"), "prlx.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "parallax", "prlxtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
