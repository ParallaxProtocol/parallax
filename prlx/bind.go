...
// Parallax network. It contains a collection of methods that are used by the
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *ParallaxClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *ParallaxClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
