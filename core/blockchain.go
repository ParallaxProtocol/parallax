...
// Package core implements the Parallax consensus protocol.
package core
...
import (
	...
	"github.com/ParallaxProtocol/parallax/internal/syncx"
...
type BlockChain struct {
	...
	db     prldb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Parallax Validator
// and Processor.
func NewBlockChain(db prldb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (xhash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db prldb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db prldb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			if batch.ValueSize() > prldb.IdealBatchSize || i == len(blockChain)-1 {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - prldb.IdealBatchSize)
			}
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		...
		if len(oldChain) > 63 {
			msg = "‼️ Large chain reorg detected"
			logFn = log.Warn
		}
