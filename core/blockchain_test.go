...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (prldb.Database, *BlockChain, error) {
	var (
	...
	blockchain, _ := NewBlockChain(db, nil, params.AllXHashProtocolChanges, engine, vm.Config{}, nil, nil)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, blockchain2, err := newCanonical(xhash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, xhash.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, xhash.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	_, blockchain, err := newCanonical(xhash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, xhash.NewFullFaker(), blockchain.db, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, processor, err := newCanonical(xhash.NewFaker(), length, full)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, blockchain, err := newCanonical(xhash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, xhash.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, xhash.NewFaker(), db, forkSeed)[1:]
...
func testReorgLong(t *testing.T, full bool) {
	...
	easy := make([]int64, 51)
	for i := 0; i < 50; i++ {
		easy[i] = int64(xhash.BlockTargetSpacingSeconds)
	}
	easy[50] = int64(xhash.BlockTargetSpacingSeconds * 2) // slow block, triggers retarget to lower diff

	hard := make([]int64, 51)
	for i := 0; i < 50; i++ {
		hard[i] = int64(xhash.BlockTargetSpacingSeconds)
	}
	hard[50] = int64(xhash.BlockTargetSpacingSeconds / 2) // fast block, triggers retarget to higher diff
...
func testReorgShort(t *testing.T, full bool) {
	...
	heavy := make([]int64, 51)
	for i := 0; i < 50; i++ {
		heavy[i] = int64(xhash.BlockTargetSpacingSeconds)
	}
	heavy[50] = int64(xhash.BlockTargetSpacingSeconds / 2) // last block triggers retarget to higher diff
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	db, blockchain, err := newCanonical(xhash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), xhash.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
		if b.header.Number.Uint64()%params.TestChainConfig.XHash.RetargetIntervalBlocks == 0 {
			b.header.EpochStartTime = b.header.Time
		} else if b.parent != nil {
			b.header.EpochStartTime = b.parent.Header().EpochStartTime
		}
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), xhash.NewFaker(), db, len(second), func(i int, b *BlockGen) {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.CurrentBlock(), 3, xhash.NewFaker(), db, 10)
		...
		headers := makeHeaderChain(blockchain.CurrentHeader(), 3, xhash.NewFaker(), db, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, xhash.NewFaker(), db, 10)
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, xhash.NewFaker(), db, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, xhash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, xhash.NewFaker(), db, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = xhash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, xhash.NewFaker(), db, 0)
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), gendb, int(height), nil)

	// makeDb creates a db instance for testing.
	makeDb := func() prldb.Database {
		dir := t.TempDir()
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, xhash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, xhash.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
...
// This PVM code generates a log when the contract is created.
...
func TestSideLogRebirth(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, xhash.NewFaker(), db, 4032, func(i int, gen *BlockGen) {
		if i == 1 {
			gen.OffsetTime(-9) // higher block difficulty
		}
	})
	...
	sideChain, _ := GenerateChain(params.TestChainConfig, genesis, xhash.NewFaker(), db, 4032, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, gen.header.BaseFee, logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, sideChain[len(sideChain)-1], xhash.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
func TestReorgSideEvent(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), db, 11, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(chain); err != nil {
		t.Fatalf("failed to insert chain: %v", err)
	}

	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), db, 11, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, xhash.NewFaker(), blockchain.db, 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	var (
		...
		gspec      = &Genesis{
			Config: &params.ChainConfig{ChainID: big.NewInt(1), EIP150Block: big.NewInt(0), EIP155Block: big.NewInt(2), HomesteadBlock: new(big.Int), XHash: &params.XHashConfig{CoinbaseMaturityBlocks: 0, RetargetIntervalBlocks: 10}},
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
	...
	config := &params.ChainConfig{ChainID: big.NewInt(2), EIP150Block: big.NewInt(0), EIP155Block: big.NewInt(2), HomesteadBlock: new(big.Int), XHash: &params.XHashConfig{CoinbaseMaturityBlocks: 0, RetargetIntervalBlocks: 10}}
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], xhash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	var (
		...
		gspec   = &Genesis{
			Config: &params.ChainConfig{
				...
				XHash:          &params.XHashConfig{CoinbaseMaturityBlocks: 0, RetargetIntervalBlocks: 10},
			},
			Alloc: GenesisAlloc{address: {Balance: funds}},
		}
		genesis = gspec.MustCommit(db)
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), db, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
//
// https://github.com/ParallaxProtocol/parallax/pull/15941
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	defer ancientDb.Close()
	gspec := Genesis{Config: params.AllXHashProtocolChanges}
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec.Config, xhash.NewFaker(), vm.Config{}, nil, nil)
...
//
// Details at:
//   - https://github.com/ParallaxProtocol/parallax/issues/18977
//   - https://github.com/ParallaxProtocol/parallax/pull/18988
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	// Generate a canonical chain to act as the main dataset
	var (
		genEngine = xhash.NewFaker()
		runEngine = xhash.NewFaker()
		db        = rawdb.NewMemoryDatabase()
...
func TestTransactionIndices(t *testing.T) {
	...
	height := uint64(128)
	blocks, receipts := GenerateChain(gspec.Config, genesis, xhash.NewFaker(), gendb, int(height), func(i int, block *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee, nil), signer, key)
		if err != nil {
			panic(err)
		}
	...
	blocks2, _ := GenerateChain(gspec.Config, blocks[len(blocks)-1], xhash.NewFaker(), gendb, 10, nil)
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, xhash.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
//     with 0.001 laxes (thus created but no code)
