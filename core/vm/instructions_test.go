...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	var (
		env            = NewPVM(BlockContext{}, TxContext{}, nil, params.TestChainConfig, Config{})
		stack          = newstack()
		pc             = uint64(0)
		pvmInterpreter = env.interpreter
	)
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, pvmInterpreter, &ScopeContext{nil, stack, nil})
		if len(stack.data) != 1 {
...
func TestAddMod(t *testing.T) {
	var (
		env            = NewPVM(BlockContext{}, TxContext{}, nil, params.TestChainConfig, Config{})
		stack          = newstack()
		pvmInterpreter = NewPVMInterpreter(env, env.Config)
		pc             = uint64(0)
	)
	tests := []struct {
		x        string
	...
	for i, test := range tests {
		...
		stack.push(y)
		stack.push(x)
		opAddmod(&pc, pvmInterpreter, &ScopeContext{nil, stack, nil})
		actual := stack.pop()
		if actual.Cmp(expected) != 0 {
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	env.interpreter = pvmInterpreter
	// convert args
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, pvmInterpreter, &ScopeContext{mem, stack, nil})
...
func BenchmarkOpKeccak256(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		stack.push(start)
		opKeccak256(&pc, pvmInterpreter, &ScopeContext{mem, stack, nil})
	}
}
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		var (
			env            = NewPVM(BlockContext{Random: &tt.random}, TxContext{}, nil, params.TestChainConfig, Config{})
			stack          = newstack()
			pc             = uint64(0)
			pvmInterpreter = env.interpreter
		)
		opRandom(&pc, pvmInterpreter, &ScopeContext{nil, stack, nil})
		if len(stack.data) != 1 {
