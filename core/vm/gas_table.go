...
func gasSStore(pvm *PVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if pvm.chainRules.IsPetersburg || !pvm.chainRules.IsConstantinople {
		...
		case current != (common.Hash{}) && y.Sign() == 0: // non 0 => 0
			pvm.StateDB.AddRefund(params.SstoreRefundGas)
	...
	if original == current {
		...
		if value == (common.Hash{}) { // delete slot (2.1.2b)
			pvm.StateDB.AddRefund(params.NetSstoreClearRefund)
		}
		return params.NetSstoreCleanGas, nil // write existing slot (2.1.2)
	}
	if original != (common.Hash{}) {
		if current == (common.Hash{}) { // recreate slot (2.2.1.1)
			pvm.StateDB.SubRefund(params.NetSstoreClearRefund)
	...
	if original == value {
		if original == (common.Hash{}) { // reset to original inexistent slot (2.2.2.1)
			pvm.StateDB.AddRefund(params.NetSstoreResetClearRefund)
		} else { // reset to original existing slot (2.2.2.2)
			pvm.StateDB.AddRefund(params.NetSstoreResetRefund)
		}
	}
...
func gasSStoreEIP2200(pvm *PVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if original == current {
		...
		if value == (common.Hash{}) { // delete slot (2.1.2b)
			pvm.StateDB.AddRefund(params.SstoreClearsScheduleRefundEIP2200)
		}
		return params.SstoreResetGasEIP2200, nil // write existing slot (2.1.2)
	}
	if original != (common.Hash{}) {
		if current == (common.Hash{}) { // recreate slot (2.2.1.1)
			pvm.StateDB.SubRefund(params.SstoreClearsScheduleRefundEIP2200)
		} else if value == (common.Hash{}) { // delete slot (2.2.1.2)
			pvm.StateDB.AddRefund(params.SstoreClearsScheduleRefundEIP2200)
		}
	}
