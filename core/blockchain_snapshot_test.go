...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllXHashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Prlx normally.
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, params.AllXHashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Prlx after the crash.
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, params.AllXHashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, params.AllXHashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, params.AllXHashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
// Tests a Prlx restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Prlx was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Prlx was crashed and restarts with a broken snapshot. In this case
...
// Tests a Prlx was running with snapshot enabled. Then restarts without
...
// Tests the Prlx was running with snapshot enabled and resetHead is applied.
...
// Tests the Prlx was running with a complete snapshot and then imports a few
