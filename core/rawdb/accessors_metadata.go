...
func ReadDatabaseVersion(db prldb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db prldb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db prldb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db prldb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadGenesisState(db prldb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisKey(hash))
	return data
}

// WriteGenesisState writes the genesis state into the disk.
func WriteGenesisState(db prldb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db prldb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db prldb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db prldb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db prldb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db prldb.KeyValueWriter, data []byte) {
