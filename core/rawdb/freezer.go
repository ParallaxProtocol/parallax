...
//     of Prlx, and thus also GC overhead.
type Freezer struct {
...
func (f *Freezer) ReadAncients(fn func(prldb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(prldb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
