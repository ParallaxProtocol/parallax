...
func NewPruner(db prldb.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb prldb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db prldb.Database, trieCachePath string) error {
...
func extractGenesis(db prldb.Database, stateBloom *stateBloom) error {
