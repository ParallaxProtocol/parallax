...
func CheckDanglingStorage(chaindb prldb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb prldb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db prldb.KeyValueStore) error {
	var (
		start   = time.Now()
