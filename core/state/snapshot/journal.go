...
func loadAndParseJournal(db prldb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Prlx
...
func loadSnapshot(diskdb prldb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Prlx was crashed without persisting journal and then
