...
func generateSnapshot(diskdb prldb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db prldb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (dl *diskLayer) generateRange(ctx *generatorContext, root common.Hash, prefix []byte, kind string, origin []byte, max int, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	var snapNodeCache prldb.KeyValueStore
	if len(result.keys) > 0 {
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > prldb.IdealBatchSize || abort != nil {
