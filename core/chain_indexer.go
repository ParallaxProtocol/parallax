...
type ChainIndexer struct {
	chainDb  prldb.Database      // Chain database to index the data from
	indexDb  prldb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb prldb.Database, indexDb prldb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
