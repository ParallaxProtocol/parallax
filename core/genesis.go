...
type Genesis struct {
	...
	EpochStartTime uint64              `json:"epochStartTime"`
...
func (ga *GenesisAlloc) flush(db prldb.Database) (common.Hash, error) {
...
func (ga *GenesisAlloc) write(db prldb.KeyValueWriter, hash common.Hash) error {
...
func CommitGenesisState(db prldb.Database, hash common.Hash) error {
...
type genesisSpecMarshaling struct {
	...
	EpochStartTime math.HexOrDecimal64
...
func (g *Genesis) ToBlock(db prldb.Database) *types.Block {
	if db == nil {
	...
	head := &types.Header{
		...
		Time:           g.Timestamp,
		EpochStartTime: g.EpochStartTime,
...
func (g *Genesis) Commit(db prldb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db prldb.Database) *types.Block {
...
func GenesisBlockForTesting(db prldb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{
...
// DefaultGenesisBlock returns the Parallax mainnet genesis block.
