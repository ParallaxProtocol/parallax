...
type Sync struct {
	database prldb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database prldb.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
// both parallax/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw prldb.Batch) error {
