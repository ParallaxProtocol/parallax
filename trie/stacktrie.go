...
func stackTrieFromPool(db prldb.KeyValueWriter) *StackTrie {
...
type StackTrie struct {
	...
	db       prldb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db prldb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewFromBinary(data []byte, db prldb.KeyValueWriter) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db prldb.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(key, val []byte, db prldb.KeyValueWriter) *StackTrie {
...
func newExt(key []byte, child *StackTrie, db prldb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
