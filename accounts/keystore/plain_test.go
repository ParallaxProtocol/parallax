...
func TestImportWalletKey(t *testing.T) {
	...
	pass := "foo"
	account, _, err := importWalletKey(ks, []byte(fileContent), pass)
	if err != nil {
		t.Fatal(err)
	}
...
// Test and utils for the key store tests in the Parallax JSON tests;
