...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Parallax contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
...
func doInstall(cmdline []string) {
	...
	// ---- Determine target OS/ARCH (from environment if provided) ----
	...
	// Build command with flags tailored to the *target* OS.
	gobuild := tc.Go("build", buildFlags(env, targetOS)...)
	gobuild.Args = append(gobuild.Args, "-trimpath", "-v")

	// ---- Construct clean env and FORCE cross-compile triple ----
	base := []string{}
	...
	// Output path respects target OS (adds .exe on windows)
	for _, pkg := range packages {
		args := append([]string{}, gobuild.Args...)
...
func buildFlags(env build.Environment, targetOS string) (flags []string) {
	var ld []string
	if env.Commit != "" {
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Parallax Builds <andrepatta@mstackb.com>"
	}
	return debMetadata{
