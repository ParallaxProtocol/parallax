...
type accountTask struct {
	...
	genBatch prldb.Batch     // Batch used by the node generator
...
// Syncer is an Parallax account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db prldb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        prldb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the Parallax state over the
// snap protocol.
func NewSyncer(db prldb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = prldb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = prldb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > prldb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > prldb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > prldb.IdealBatchSize {
