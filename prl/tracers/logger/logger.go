...
// Config are the configuration options for structured logger the PVM
type Config struct {
...
// StructLog is emitted to the PVM each cycle and lists information about the current internal state
...
// StructLogger is an PVM state logger and implements PVMLogger.
//
...
func (l *StructLogger) CaptureStart(env *vm.PVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	l.env = env
}
...
func (l *StructLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	// create a new snapshot of the PVM.
...
func (t *mdLogger) CaptureStart(env *vm.PVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	if !create {
...
// ExecutionResult groups all structured logs emitted by the PVM
...
// StructLogRes stores a structured log emitted by the PVM while replaying a
...
// formatLogs formats PVM returned structured logs for json output
