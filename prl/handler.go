...
// support all the operations needed by the Parallax chain protocols.
...
// newHandler returns a handler for all Parallax chain management protocol.
...
func (h *handler) runParallaxPeer(peer *prl.Peer, handler prl.Handler) error {
	...
	peer.Log().Debug("Parallax peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Parallax peer registration failed", "err", err)
		return err
	}
	...
	if h.checkpointHash != (common.Hash{}) {
		...
		go func() {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*prl.BlockHeadersPacket))
				if len(headers) == 0 {
...
func (h *handler) unregisterPeer(id string) {
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Parallax peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Parallax protocol stopped")
}
