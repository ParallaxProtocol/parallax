...
type LightPeer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *prl.Response) (*prl.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *prl.Response) (*prl.Request, error)
}

// Peer encapsulates the methods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
	RequestBodies([]common.Hash, chan *prl.Response) (*prl.Request, error)
	RequestReceipts([]common.Hash, chan *prl.Response) (*prl.Request, error)
}
...
func (w *lightPeerWrapper) RequestHeadersByHash(h common.Hash, amount int, skip int, reverse bool, sink chan *prl.Response) (*prl.Request, error) {
	return w.peer.RequestHeadersByHash(h, amount, skip, reverse, sink)
}

func (w *lightPeerWrapper) RequestHeadersByNumber(i uint64, amount int, skip int, reverse bool, sink chan *prl.Response) (*prl.Request, error) {
	return w.peer.RequestHeadersByNumber(i, amount, skip, reverse, sink)
}

func (w *lightPeerWrapper) RequestBodies([]common.Hash, chan *prl.Response) (*prl.Request, error) {
	panic("RequestBodies not supported in light client mode sync")
}

func (w *lightPeerWrapper) RequestReceipts([]common.Hash, chan *prl.Response) (*prl.Request, error) {
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(prl.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(prl.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(prl.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(prl.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(prl.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(prl.ReceiptsMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "parallax")),
	}
}
