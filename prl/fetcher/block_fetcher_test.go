...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *prl.Response) (*prl.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *prl.Response) (*prl.Request, error) {
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash common.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash common.Hash, sink chan *prl.Response) (*prl.Request, error) {
		atomic.AddUint32(&counter, 1)

		// Simulate a long running fetch
		resink := make(chan *prl.Response)
