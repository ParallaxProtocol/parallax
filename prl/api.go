...
// PublicParallaxAPI provides an API to access Parallax full node-related
// information.
type PublicParallaxAPI struct {
	e *Parallax
}

// NewPublicParallaxAPI creates a new Parallax protocol API for full nodes.
func NewPublicParallaxAPI(e *Parallax) *PublicParallaxAPI {
	return &PublicParallaxAPI{e}
}

// Coinbase is the address that mining rewards will be send to
func (api *PublicParallaxAPI) Coinbase() (common.Address, error) {
	return api.e.Coinbase()
}

// Hashrate returns the POW hashrate
func (api *PublicParallaxAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Parallax) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Parallax) *PrivateMinerAPI {
...
// SetCoinbase sets the address of the miner
...
// PrivateAdminAPI is the collection of Parallax full node-related APIs
...
// admin methods of the Parallax service.
func NewPrivateAdminAPI(prl *Parallax) *PrivateAdminAPI {
	return &PrivateAdminAPI{prl: prl}
}
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.prl.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.prl.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.prl.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.prl.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.prl.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Parallax full node APIs exposed
...
// related public debug methods of the Parallax service.
func NewPublicDebugAPI(prl *Parallax) *PublicDebugAPI {
	return &PublicDebugAPI{prl: prl}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.prl.miner.Pending()
	...
	if blockNr == rpc.LatestBlockNumber {
		...
		block = api.prl.blockchain.CurrentFinalizedBlock()
	} else {
		block = api.prl.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.prl.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Parallax full node APIs exposed over
...
// private debug methods of the Parallax service.
func NewPrivateDebugAPI(prl *Parallax) *PrivateDebugAPI {
	return &PrivateDebugAPI{prl: prl}
}
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			if number == rpc.LatestBlockNumber {
				...
				block = api.prl.blockchain.GetBlockByNumber(uint64(number))
			}
			if block == nil {
		...
		block := api.prl.blockchain.GetBlockByHash(hash)
		if block == nil {
