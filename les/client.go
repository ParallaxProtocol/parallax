...
// Package les implements the Light Parallax Subprotocol.
package les
...
func (s *LightParallax) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightParallax) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightParallax) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
func (s *LightParallax) ResetWithGenesisBlock(gb *types.Block) {
...
// Parallax protocol.
func (s *LightParallax) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
