...
import (
	...
	"github.com/ParallaxProtocol/parallax/les/fetcher"
...
// the prl.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb prldb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
//
// Note, we rely on the underlying prl/fetcher to retrieve and validate the
// response, so that we have to obey the rule of prl/fetcher which only accepts
