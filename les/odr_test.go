...
type odrTestFn func(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte
...
func odrGetBlock(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrGetReceipts(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrAccounts(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func odrContractCall(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				context := core.NewPVMBlockContext(header, bc, nil)
				txContext := core.NewPVMTxContext(msg)
				vmenv := vm.NewPVM(context, txContext, statedb, config, vm.Config{NoBaseFee: true})
			...
			context := core.NewPVMBlockContext(header, lc, nil)
			txContext := core.NewPVMTxContext(msg)
			vmenv := vm.NewPVM(context, txContext, state, config, vm.Config{NoBaseFee: true})
			gp := new(core.GasPool).AddGas(math.MaxUint64)
			result, _ := core.ApplyMessage(vmenv, msg, gp)
			if state.Error() == nil {
				res = append(res, result.Return()...)
...
func odrTxStatus(ctx context.Context, db prldb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func TestGetTxStatusFromUnindexedPeersLPS4(t *testing.T) { testGetTxStatusFromUnindexedPeers(t, lpv4) }
