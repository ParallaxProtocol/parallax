...
const (
	...
	// handshakeTimeout is the timeout LPS handshake will be treated as failed.
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Parallax protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local parallax node is running in archive mode, advertise ourselves we have
...
// participating in the Light Parallax sub-protocol.
...
// also run the LPS protocol. We can't drop them although they are useless
// for us(server) but for other protocols(e.g. PRL) upon the devp2p they
// may be useful.
