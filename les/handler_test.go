...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), xhash.NewFaker(), server.db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), xhash.NewFaker(), server.db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
...
func TestStopResumeLPS3(t *testing.T) { testStopResume(t, lpv3) }
func TestStopResumeLPS4(t *testing.T) { testStopResume(t, lpv4) }
