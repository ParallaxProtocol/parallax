...
func testSequentialAnnouncements(t *testing.T, protocol int) {
	...
	// Create connected peer pair, the initial signal from LPS server
...
func testGappedAnnouncements(t *testing.T, protocol int) {
	...
	blocks, _ := core.GenerateChain(rawdb.ReadChainConfig(s.db, s.backend.Blockchain().Genesis().Hash()), s.backend.Blockchain().GetBlockByNumber(3),
		xhash.NewFaker(), s.db, 2, func(i int, gen *core.BlockGen) {
...
func TestInvalidAnnouncesLPS2(t *testing.T) { testInvalidAnnounces(t, lpv2) }
func TestInvalidAnnouncesLPS3(t *testing.T) { testInvalidAnnounces(t, lpv3) }
func TestInvalidAnnouncesLPS4(t *testing.T) { testInvalidAnnounces(t, lpv4) }
