...
func NewLesServer(node *node.Node, e prlBackend, config *prlconfig.Config) (*LesServer, error) {
	...
	// LPS flow control tries to more or less guarantee the possibility for the
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Parallax light client service")
...
func (s *LesServer) Protocols() []p2p.Protocol {
	...
	// Add "les" ENR entries.
	for i := range ps {
		ps[i].Attributes = []enr.Entry{&lpsEntry{
			VfxVersion: 1,
		}}
	}
	return ps
