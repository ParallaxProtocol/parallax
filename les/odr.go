...
func NewLesOdr(db prldb.Database, config *light.IndexerConfig, peers *serverPeerSet, retriever *retrieveManager) *LesOdr {
	return &LesOdr{
		db:            db,
...
func (odr *LesOdr) Database() prldb.Database {
	return odr.db
}
...
// Msg encodes a LPS message that delivers reply data for a request
type Msg struct {
	MsgType int
	ReqID   uint64
	Obj     any
...
// RetrieveTxStatus retrieves the transaction status from the LPS network.
// There is no guarantee in the LPS protocol that the mined transaction will
...
// Retrieve tries to fetch an object from the LPS network. It's a common API
// for most of the LPS requests except for the TxStatusRequest which needs
