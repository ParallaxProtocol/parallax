...
import (
	...
	"github.com/ParallaxProtocol/parallax/les/checkpointoracle"
	"github.com/ParallaxProtocol/parallax/les/flowcontrol"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some laxes to user1
		...
		case 1:
			...
			// bankUser transfers more laxes to user1
		...
		case 2:
			...
			// bankUser transfer some laxes to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db prldb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db prldb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) (*clientHandler, func()) {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db prldb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend, func()) {
	var (
		gspec = core.Genesis{
			Config:   params.AllXHashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
