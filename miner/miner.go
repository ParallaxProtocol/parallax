...
type Config struct {
	...
	Notify     []string       `toml:",omitempty"` // HTTP URL list to be notified of new work packages (only useful in xhash).
	...
	Noverify   bool           // Disable remote mining solution verification(only useful in xhash).
}
...
func New(parallax Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool) *Miner {
	miner := &Miner{
		parallax: parallax,
		mux:      mux,
		engine:   engine,
		...
		worker:   newWorker(config, chainConfig, engine, parallax, mux, isLocalBlock, true),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
