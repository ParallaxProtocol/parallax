...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data any) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Parallax PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useParallaxV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Parallax ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Ethereum Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useParallaxV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
