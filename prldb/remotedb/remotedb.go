...
func (db *Database) ReadAncients(fn func(op prldb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(prldb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func (db *Database) NewBatch() prldb.Batch {
	panic("not supported")
}

func (db *Database) NewBatchWithSize(size int) prldb.Batch {
	panic("not supported")
}

func (db *Database) NewIterator(prefix []byte, start []byte) prldb.Iterator {
	panic("not supported")
}
...
func (db *Database) NewSnapshot() (prldb.Snapshot, error) {
	panic("not supported")
}
...
func New(endpoint string) (prldb.Database, error) {
